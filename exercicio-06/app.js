...
for(let i = 0; i < crazyArray.length; i++){
  const tipo = typeof crazyArray[i];
  const booleano = tipo === 'boolean';
  const numero = tipo === 'number';
...
const arrayStringPar = numerosPares.join(', ').replace(', 3', ' e 3');
const arrayStringImpar = numerosImpares.join(', ').replace(', 5', ' e 5');

console.log(`Numeros ímpares: ${arrayStringImpar}. Números pares: ${arrayStringPar}`);
